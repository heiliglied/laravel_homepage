				...
				<div class="row">
					<div class="col-md-12">
						<div class="card card-primary">
							...
							<form name="rank_form" method="post" action="/admin/settings/permission/update" onSubmit="return modify_permission(this)">
							{{ csrf_field() }}
							@method('PATCH')
							...
							<div class="card-body">
								<div class="form-group">
									...
									<select name="rank" class="form-control" id="rank" required>
										...
										<option value="{{ $admin_rank->rank }}" @if($admin_rank->rank == $permission->rank) selected @endif>{{ $admin_rank->name }}</option>
										@endforeach
									</select>
								</div>
								<div class="form-group">
									<label for="uri">URI </label>
									<input type="text" name="uri" value="{{ $permission->uri }}" id="uri" class="form-control" required placeholder="권한을 등록할 URI를 주소창에서 복사하여 기록해 주세요.">
									<div id="uri_status">
									</div>
								</div>
								<div class="row">
...
function modify_permission(form) {
	if(form.uri.value == '') {
